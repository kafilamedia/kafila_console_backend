...
@section('content')
    ...
    <h1 class="title">
	 ...
	 <div class="level">
        ...
        <div class="level-right">
            @if($soal_essay->count() > 0 && count(Arr::get($submission, 'nilai_essay', [])) == $soal_essay->count() && ! $submission->released)
                <form action="{{ route('admin.lock-score') }}" method="POST">
                    @csrf
                    <input type="hidden" name="submisi_id" value="{{ $submission->id }}">
                    <button class="button is-{{ $submission->score_locked ? 'danger' : 'dark' }}">
                        <span class="icon">
                            <i class="fas fa-lock"></i>
                        </span>
                        @if($submission->score_locked)
    ...
    <div class="box">
        <h3 class="title is-4">{{ $student->name }}</h3>
    </div>
    ...
    @foreach(Arr::get($cbt->soal->groupBy('jenis'), 'es', []) as $item)
        ...
        <div class="box">
            <div class="level">
                ...
                <div class="level-right">
                    {{-- <pre>{{ var_dump($item->urut, $submission->nilai_essay) }}</pre> --}}
                    @if(in_array($item->urut, array_keys(Arr::get($submission, 'nilai_essay', []))))
	        ...
	        <div class="columns" style="margin-top: 10px;">
	        	<div class="column">
                    <x-kartu>
                        <x-slot name="title">Jawaban Siswa</x-slot>
                         @php
                            $jawaban_essay = Arr::get($submission->jawaban, Arr::get(array_flip($submission->nomor_urut_teracak), $item->urut));
                        @endphp
	        	...
	        	<div class="column">
                    <x-kartu>
                        <x-slot name="title">Kunci Jawaban</x-slot>
                        {!! $item->jawaban !!}
                    </x-kartu>
	        	</div>
	        </div>
            ...
            @if($submission->score_locked)
                <div class="tags has-addons">
                    ...
                    <span class="tag is-medium is-success">{{ Arr::get($submission->nilai_essay, $item->urut, 0) }}</span>
                </div>
            @else
                <form action="{{ route('admin.essay-score') }}" method="POST">
                    @csrf
                    <div class="field is-horizontal">
                        ...
                        <div class="field-body">
                            <div class="field is-narrow">
                                <div class="control">
                                    <input type="hidden" name="submisi_id" value="{{ $submission->id }}">
                                    <input name="nilai_essay[{{ $item->urut }}]" class="input{{ $errors->has('nilai_essay.' . $item->urut) ? ' is-danger' : '' }}" type="number" placeholder="Nilai / 100" value="{{ old('nilai_essay.' . $item->urut, Arr::get($submission->nilai_essay, $item->urut, 0)) }}">
                                </div>
                	...
                	<div class="field is-grouped">
	                	<div class="control">
	                		<div class="tags has-addons">
		                        ...
		                        <span class="tag is-medium is-info">{{ strtoupper(Arr::get($submission->jawaban, Arr::get(array_flip($submission->nomor_urut_teracak), $item->urut))) }}</span>
		                    </div>
	                	</div>
	                	<div class="control">
	                		<div class="tags has-addons">
                    ...
                    <article class="media">
                        <figure class="media-left">
                            @php
                                ...
                                if (in_array($item->urut, Arr::get($submission, 'nomor_urut_jawaban_salah', [])) && $key == Arr::get($submission->jawaban, Arr::get(array_flip($submission->nomor_urut_teracak) ,$item->urut))) {
